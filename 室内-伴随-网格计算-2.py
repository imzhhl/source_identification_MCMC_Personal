...
def likelihood_func_log(sigma, c_predict, c_true):
    ...
    temp = 0
    for i in range(len(c_predict)):   
        temp = temp + (-np.log(sigma * np.sqrt(2* np.pi) )-((c_true[i]-c_predict[i])**2) / (2*sigma**2))
    return np.exp(temp) 
...
def file_to_array(points_list):
    ...
    with open(r'F:/ZHHL/TE_Doctor/CASES/case220927/fluent16-uniformmesh-0928_1','r')  as  csvfile:   
    ...
    np.save('uds_adjoint_1.npy', uds_t)
...
# 定义预测值
c_predict = np.zeros([len(point_true_value),3000,9000])

c_predict = adjoint_results()

# %%计算似然函数
sigma = likelihood_func_log(0.03, c_predict, point_true_value)
...
# 污染物扩散二维等值线图
# matplotlib.pyplot.contour(X, Y, c_predict[0,:,:], colors=list(["purple","blue","cyan", "green","yellow","orange","red"]), levels = [0.02, 0.05, 0.1, 0.2, 0.3, 0.4], linestyles=['-'])

# 二维后验密度图
plt.contourf(X, Y, sigma,offset = -2, cmap = plt.cm.rainbow, linestyles=['-'])
...
surf = ax.plot_surface(X, Y, sigma, linestyle="solid", rstride=50, cstride=50, edgecolor='black',  cmap='rainbow')
...
# 设置label
ax.set_xlabel( '$X$',font,labelpad=(40) )
ax.set_ylabel( '$Y$',font,labelpad=(10) )
ax.set_zlabel( '$P$',fontdict = font, labelpad=(10) )
x_major_locator=MultipleLocator(1)
#把x轴的刻度间隔设置为1，并存在变量里
y_major_locator=MultipleLocator(1)
#把y轴的刻度间隔设置为10，并存在变量里
