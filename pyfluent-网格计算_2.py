...
def get_points_value_IO(points_list):
    ...
    for i in range(len(points_list)):
        ...
        if i < len(points_list)-1:
            s = s + ' '
    index_1 = '(' + s + ')'
    points_file = base_path + r'\interactive_files\points_value-2'
...
# 用于返回监测点处的浓度值
def direct_fluent(X_current = 0.0, Y_current = 0.0, points_list = []): 
    #------------------------------------------------------------------------------
...
point_true_value = direct_fluent(X_true, Y_true, points_list)
...
Y = np.linspace(-250, 250, 501)
X, Y = np.meshgrid(X, Y)

# 定义预测值
c_predict = np.zeros([9,501,1001])

################################### 迭代开始 ###################################
start = time.perf_counter() 
for i in tqdm(range(501)):
    for j in range(1001):      
        value = direct_fluent(X[i, j], Y[i, j], points_list)
        for z in range(9):
...
# %%计算似然函数
sigma = likelihood_func(0.04, c_predict, point_true_value)
